...
> = {
  ...
  12: {
    ...
    aura: "from-rose-950 via-purple-950 to-slate-950",
  },
  13: {
    label: "秩序之環 Totem",
...
const dimensionVisuals: Record<
  DimensionCode,
  {
    vignetteClass: string; // 中央暈光
    particleClass: string; // 粒子顏色
    cardGlowClass: string; // 卡片外圈光暈
  }
> = {
  EI: {
    vignetteClass:
...
export default function QuizPage() {
  ...
  const qVis = questionVisuals[currentQuestion.id] ?? questionVisuals[1];
  ...
  return (
    <div
      className={`min-h-screen bg-gradient-to-b ${qVis.aura} text-slate-100 flex items-center justify-center px-4 py-10 relative overflow-hidden`}
    >
      {/* ===== 背景森林層 ===== */}
      <div className="pointer-events-none absolute inset-0 -z-30">
        ...
        {/* 大型光束：整體氛圍 */}
        <motion.div
          className="absolute -left-40 -top-40 h-80 w-80 bg-gradient-to-br from-teal-400/20 via-cyan-300/10 to-transparent blur-3xl"
          animate={{
            x: [0, 40, 0],
            y: [0, 20, 0],
            rotate: [0, 8, -4, 0],
        ...
        />
        <motion.div
          className="absolute -right-40 bottom-[-120px] h-80 w-80 bg-gradient-to-tr from-emerald-400/20 via-sky-300/10 to-transparent blur-3xl"
          animate={{
            x: [0, -30, 0],
            y: [0, -18, 0],
            rotate: [0, -6, 3, 0],
        ...
        {isFinal && (
          <motion.div
            className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(250,249,222,0.55),_transparent)] mix-blend-screen"
            initial={{ opacity: 0 }}
            animate={{ opacity: [0.2, 0.9, 0.3, 1, 0.4] }}
            transition={{
              duration: 5,
              repeat: Infinity,
              repeatType: "mirror",
      ...
      >
        ...
        {/* 粒子 3 */}
        <motion.div
          className={`absolute w-2 h-2 rounded-full blur-[1px] ${dimVis.particleClass}`}
          initial={{ x: "40%", y: "12%", opacity: 0.5 }}
          animate={{
            y: ["12%", "62%", "18%"],
            opacity: [0.5, 1, 0.5],
          }}
      ...
      >
        {/* 卡片本體 */}
        <motion.div
          className={`relative rounded-3xl border border-slate-700/80 bg-slate-950/85 backdrop-blur-xl overflow-hidden ${dimVis.cardGlowClass}`}
          animate={{
        ...
        >
          ...
          {isFinal && (
            <motion.div
              className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(250,250,255,0.28),_transparent)] mix-blend-screen"
              initial={{ opacity: 0.2 }}
          ...
          {/* 進度條 */}
          <div className="px-6 pt-5 pb-3 border-b border-slate-700/60 bg-slate-950/80 relative z-10">
          ...
          <div className="px-6 py-6 sm:py-8 relative z-10">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                >
                  <motion.div
                    className="h-12 w-12 rounded-full border border-teal-300/70 bg-slate-900/95 shadow-[0_0_25px_rgba(45,212,191,0.8)] overflow-hidden flex items-center justify-center"
                    animate={{
                      y: [0, -4, 0],
                      rotate: [-2, 2, -2],
                    }}
                ...
                <div className="mt-4 space-y-3">
                  {currentQuestion.options.map((opt, idx) => (
                    <motion.button
                      ...
                      className="w-full text-left px-4 py-3.5 rounded-2xl bg-slate-900/85 hover:bg-slate-800/95 border border-slate-700/90 hover:border-teal-400/80 transition-all duration-200 shadow-[0_18px_30px_rgba(0,0,0,0.85)]"
                    >
